...
import { getDatabase, ref, query, orderByChild, equalTo, onValue } from "https://www.gstatic.com/firebasejs/12.7.0/firebase-database.js";
...
function isOpen() { return document.getElementById("modalBackdrop")?.classList.contains("open"); }
...
function openProductModal(p) {
  ...
  if (specsEl) specsEl.innerHTML = (p.specs || []).map((s) => `<span class="badge">${escapeHtml(s)}</span>`).join("");
...
function setCardIndex(st, idx) {
  ...
  if (st.dotsEl) [...st.dotsEl.children].forEach((d, i) => d.classList.toggle("active", i === st.idx));
}
function buildCardDots(st) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    b.type = "button"; b.className = "cardDot " + (i === 0 ? "active" : "");
    b.setAttribute("aria-label", `Фото ${i + 1} из ${n}`);
    b.addEventListener("click", (e) => { e.stopPropagation(); setCardIndex(st, i); });
    st.dotsEl.appendChild(b);
  }
}
function attachSwipe(st, targetEl) {
  let downX = 0, downY = 0, moved = false;
  targetEl.addEventListener("pointerdown", (e) => { downX = e.clientX; downY = e.clientY; moved = false; targetEl.setPointerCapture?.(e.pointerId); });
  targetEl.addEventListener("pointermove", (e) => { if (Math.abs(e.clientX - downX) > 10 && Math.abs(e.clientX - downX) > Math.abs(e.clientY - downY)) moved = true; });
  targetEl.addEventListener("pointerup", (e) => {
    const dx = e.clientX - downX;
    if (moved && Math.abs(dx) > 30 && st.images.length > 1) { st.justSwiped = true; setCardIndex(st, st.idx + (dx < 0 ? 1 : -1)); setTimeout(() => (st.justSwiped = false), 0); }
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("modalPrev")?.addEventListener("click", () => { modalIndex--; renderModalGallery(); });
  document.getElementById("modalNext")?.addEventListener("click", () => { modalIndex++; renderModalGallery(); });
  document.addEventListener("keydown", (e) => {
    if (!isOpen()) return;
    if (e.key === "Escape") closeModal();
    if (e.key === "ArrowLeft") { modalIndex--; renderModalGallery(); }
    if (e.key === "ArrowRight") { modalIndex++; renderModalGallery(); }
  });
  ...
  onValue(q, (snap) => {
    ...
    cards.innerHTML = "";
    if (!arr.length) { cards.innerHTML = `<div class="panel" style="padding:24px; text-align:center; color:var(--text-secondary)">Пока нет товаров. Следите за обновлениями!</div>`; return; }

    arr.forEach((p) => {
      const imagesRaw = Array.isArray(p?.imageUrls) && p.imageUrls.length ? p.imageUrls : (p?.imageUrl ? [p.imageUrl] : []);
      ...
      if (typeof p.qty === "number") specs.push(`${p.qty} шт`);

      const avitoUrl = (p.avitoUrl || "").trim() || AVITO_FALLBACK;
      const ozonUrl = (p.ozonUrl || "").trim() || OZON_FALLBACK;
      ...
      card.innerHTML = `
        ...
        <div class="cardBody">
          ...
          <a class="btnOrder" href="${ozonUrl}" target="_blank" rel="noopener">Заказать на Ozon</a>
        </div>
      `;
      ...
      card.addEventListener("click", () => {
        ...
        openProductModal({ title: p.title, full: p.full || p.short, specs, images: safeImages, avito: avitoUrl, ozon: ozonUrl });
      });
    });
  });
});